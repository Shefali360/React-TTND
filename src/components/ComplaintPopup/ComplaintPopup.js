      ...
      <div className={styles.popup}>
        <div className={styles.data}>
          <div className={styles.column1}>
            <span className={styles.issueId}>{complaint.issueId}</span>
            <p className={styles.status}>({complaint.status})</p>
          </div>
          <p className={styles.text}>
            Logged On:{dayFormat}/{monthFormat}/{year}
          </p>
          {complaint.status === "In Progress" ? (
            <p className={styles.text}>
              Estimated time:{complaint.estimatedTime.count}{" "}
              {complaint.estimatedTime.timeType}
            </p>
          ) : null}
          {complaint.lockedBy ? (
            <p className={styles.text}>Locked By:{complaint.lockedBy.name}</p>
          ) : null}
